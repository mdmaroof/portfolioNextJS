  ...
  <section>
    ...
    <div className="project-orbit-grid mt-10">
      {projects.map((project, index) => { const Icon = icons[project.name] || FiHexagon; return (
        <m.article key={project.name} className={`project-orbit-card project-tone-${(index % 4) + 1}`} initial={{ opacity: 0, y: 28, rotateX: 3 }} whileInView={{ opacity: 1, y: 0, rotateX: 0 }} viewport={{ once: true, amount: .15 }} transition={{ duration: .58, delay: (index % 2) * .08, ease: [0.16, 1, 0.3, 1] }}>
