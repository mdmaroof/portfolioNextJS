    ...
    <div className="project-orbit-grid mt-10">
      {projects.map((project, index) => { const Icon = icons[project.name] || FiHexagon; return (
        <m.article key={project.name} className={`project-orbit-card project-tone-${(index % 4) + 1}`} initial={{ opacity: 0, y: 28, rotateX: 3 }} whileInView={{ opacity: 1, y: 0, rotateX: 0 }} whileHover={{ y: -6 }} viewport={{ once: true, amount: .15 }} transition={{ duration: .58, delay: (index % 2) * .08, ease: [0.16, 1, 0.3, 1] }}>
          <div className="project-orbit-art" aria-hidden="true"><m.div className="project-ring project-ring-one" whileInView={{ opacity: [.45, .82, .45] }} viewport={{ amount: .18 }} transition={{ duration: 4.6 + index * .35, repeat: Infinity, ease: "easeInOut" }} /><m.div className="project-ring project-ring-two" whileInView={{ opacity: [.25, .58, .25] }} viewport={{ amount: .18 }} transition={{ duration: 6 + index * .4, repeat: Infinity, ease: "easeInOut", delay: .8 }} /><span className="project-core-anchor"><m.span className="project-core" whileInView={{ y: [0, -6, 0], rotate: [-7, 3, -7] }} viewport={{ amount: .18 }} transition={{ duration: 4.2 + index * .3, repeat: Infinity, ease: "easeInOut" }}><Icon /></m.span></span><span className="project-number">0{index + 1}</span></div>
